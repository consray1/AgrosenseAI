...
export default function Dashboard() {
  ...
  return (
    <section id="dashboard" className="py-16 md:py-24 px-4 md:px-8 lg:px-12" style={{ background: "var(--bg2)" }}>
      <div className="container mx-auto">
        <div
          className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 mb-8 md:mb-12"
        >
          <div>
          ...
          <div className="flex flex-wrap gap-2 md:gap-3 items-center">
            <span className="tag-pill pill-green text-xs md:text-sm">● Live Data</span>
            <span className="tag-pill pill-yellow text-xs md:text-sm">⚠ 3 Active Alerts</span>
            <a href="#" className="btn btn-primary text-xs md:text-sm">
              Open Dashboard →
            </a>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <span className="hidden md:inline" style={{ marginLeft: "auto", fontSize: "11px", color: "var(--muted)" }}>
...
function DashboardSidebar() {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "rgba(139,168,158,0.5)", padding: "12px 12px 6px" }}>
        Analytics
      </div>
...
function DashboardMap({
  ...
  return (
    ...
    >
      <svg viewBox="0 0 500 480" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", height: "100%", minHeight: "300px" }}>
        ...
        <ellipse cx="285" cy="100" rx="60" ry="40" fill="rgba(255,193,7,0.25)" stroke="rgba(255,193,7,0.6)" strokeWidth="1" />

        <ellipse cx="175" cy="220" rx="35" ry="50" fill="rgba(0,191,165,0.2)" stroke="rgba(0,191,165,0.5)" strokeWidth="1" />
        ...
        <text x="270" y="95" fill="rgba(255,193,7,0.9)" fontSize="9" fontWeight="700">MARSABIT</text>
        <text x="155" y="218" fill="rgba(0,191,165,0.9)" fontSize="9" fontWeight="700">TURKANA</text>
      </svg>
      ...
      >
        {layers.map((layer) => (
          <div
            ...
            style={{
              background: activeLayer.includes(layer) ? "rgba(46,125,50,0.25)" : "rgba(16,42,35,0.85)",
              ...
              borderColor: activeLayer.includes(layer) ? "var(--secondary)" : "var(--border2)",
              transition: "all 0.2s",
            }}
          >
            {layer === "Satellite" && "🛰 "}
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: "13px", color: "var(--text)", lineHeight: 1.5 }}>
          Marsabit maize farmers at <strong>grain-fill stage</strong> face critical drought stress over the next 14 days. Recommend early harvest advisory to 1,240 registered farmers.
        </div>
      </div>
      ...
      >
        ...
        {[
          ...
          <div key={item.crop} style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
            <div style={{ fontSize: "12px", color: "var(--text)", width: "60px", flexShrink: 0 }}>{item.crop}</div>
            <div style={{ flex: 1, height: "6px", background: "rgba(255,255,255,0.06)", borderRadius: "3px", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${item.risk}%`, borderRadius: "3px", background: item.color }} />
            </div>
            <div style={{ fontSize: "11px", fontWeight: 700, width: "32px", textAlign: "right", color: item.color }}>{item.risk}%</div>
          </div>
        ))}
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
        <div style={{ background: "rgba(0,0,0,0.2)", border: "1px solid var(--border)", borderRadius: "10px", padding: "12px" }}>
